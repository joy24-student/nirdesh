...
import { ArrowRight, ChevronDown, Menu, X, Sparkles, User, Download } from 'lucide-react';
...
export const Navbar: React.FC<NavbarProps> = ({ activeView, setActiveView, onOpenAuth, currentUser }) => {
  ...
  return (
    <header
      className={`fixed top-0 left-0 right-0 z-50 transition-all duration-300 ${
        scrolled
          ? 'bg-[#050813]/85 backdrop-blur-xl border-b border-white/10 py-3 shadow-2xl shadow-cyan-950/20'
          : 'bg-transparent py-5'
      }`}
    >
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex items-center justify-between">
                        ...
                        <div className="space-y-2">
                          {cat.items.map((sub) => (
                            <button
                              ...
                              className="w-full text-left p-2 rounded-xl hover:bg-cyan-500/10 border border-transparent hover:border-cyan-500/20 transition-all group"
                            >
          ...
          <div className="hidden lg:flex items-center gap-3">
            {currentUser ? (
              <div className="flex items-center gap-2 px-3 py-1.5 rounded-full glass-panel text-sm text-cyan-300">
              ...
              <button
                ...
                className="text-sm font-medium text-slate-300 hover:text-white px-4 py-2 rounded-full hover:bg-white/5 transition-all"
              >
                Sign In
              </button>
            )}
            ...
            <button
              ...
              className="flex items-center gap-2 px-5 py-2.5 text-sm font-semibold text-white rounded-full bg-nirdesh-gradient hover:opacity-95 transition-all shadow-lg shadow-cyan-500/25 hover:shadow-cyan-500/40 hover:scale-[1.02] active:scale-[0.98]"
            >

...
import { HERO_CAPABILITY_CARDS } from '../../config/siteData';
import { Brain, CheckCircle2, ShieldCheck, Zap, PieChart } from 'lucide-react';
    ...
    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-4 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 mt-12 relative z-20">
      {HERO_CAPABILITY_CARDS.map((card) => {
        ...
        return (
          ...
          >
            {/* Top icon */}
            <div className="w-12 h-12 rounded-xl bg-cyan-500/10 border border-cyan-500/20 flex items-center justify-center mb-4 group-hover:scale-110 group-hover:bg-cyan-500/20 transition-all shadow-md">
            ...
            {/* Bottom glow line */}
            <div className="absolute bottom-0 left-0 right-0 h-0.5 bg-gradient-to-r from-transparent via-cyan-500/50 to-transparent opacity-0 group-hover:opacity-100 transition-opacity" />
          </div>
        );
      })}
    </div>
